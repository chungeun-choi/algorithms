...
def solution(fees, records):
    ...
    for record in records:
        ...
        if car_number not in parking_history:
            parking_history.setdefault(
                car_number, {action: datetime.strptime(time, "%H:%M")}
            )
            ...
            if action == "OUT":
                result_time = (
                    datetime.strptime(time, "%H:%M") - parking_history[car_number]["IN"]
                )
    ...
    for car_number, value in parking_history.items():
        parking_time = (
            datetime.strptime("23:59", "%H:%M") - value["IN"]
        ).total_seconds() / 60
        pay_history[car_number] += parking_time
...
if __name__ == "__main__":
    ...
    test_inpu2 = {"fees": [1, 461, 1, 10], "records": ["00:00 1234 IN"]}
